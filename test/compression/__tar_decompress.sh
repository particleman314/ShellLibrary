...
answer_dir="$( __tar_decompress --input "${compression_file}" --keep-orig )"
assert_success $?
...
__tar_decompress --output-dir "${outputdir}"
assert_failure $?

answer_dir="$( __tar_decompress --input "${compression_file}" --output-dir "${outputdir}" --keep-orig )"
assert_success $?
